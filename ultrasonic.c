...
if (argc != 2)
{
    printf("expected an argument; enter one integer");
    return 1;
}
int rep;
int last=0;
int found = sscanf(argv[1], "%d", &rep);
if (found != 1) {
    printf("expected an integer as input");
    return 1;
}
FILE * output;
output = fopen("/var/www/html/student4/output.txt", "w");
for (int i=0; i<rep; i+=1){
        setup();
	last=getCM();
        fprintf(output, "Distance: %dcm\n ", getCM());
